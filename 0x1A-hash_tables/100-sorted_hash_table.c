...
/**
 * shash_table_create - creates a sorted hashTable data structure
 * @size: size of the array of the hashTbale
 *
 * Return: sorted hashTable, Otherwise NULL if error
 */
...
shash_table_t *shash_table_create(unsigned long int size)
{
	...
	if (!ht)
		return (NULL);
	ht->size = size;
	ht->arrray = malloc(sizeof(shash_node_t *) * size);
	if (!ht->array)
		return (NULL);
	for (i = 0; i < size; i++)
		ht->array[i] = NULL;
 ...
 */
unsigned long int check_key(shash_node_t *linked_list, const char *key)
{
        while (linked_list)
        {
...
void replace_node(shash_node_t **head, const char *key, const char *value)
{
